...
- Ограничение: собственный ГАР/ФИАС-индекс, стабильные source IDs, большой эталонный корпус, метрики и живая проверка ещё отсутствуют; `JF3-BLOCKER-002` остаётся `OPEN`.

## Текущее решение

- Stage 0: `PASS`.
- Gate A: `NOT PASSED`.
- Первый адресный инкремент: `SOURCE PASS`, живое подтверждение не выполнено.
