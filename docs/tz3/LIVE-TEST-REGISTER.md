...
- Первый запуск актуальной сборки: `PENDING CONFIRMATION`; вход владельца и пустой склад были подтверждены на предыдущем RC и должны быть повторно проверены после запуска новой установки.
...
- VPS, Telegram и живой адресный provider: `PENDING`; секреты ещё не вводились и будут вводиться владельцем лично.
