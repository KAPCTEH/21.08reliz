...
5. Windows CI собрал и проверил Setup, Recovery и payload, но артефакт ещё не скачан, не установлен и не запущен на пользовательском ПК.
6. Не выполнены живые циклы всех функций, визуальная проверка и несколько физических ПК.
...
- GitHub Audit incremental `32624837813`: 64 из 64 безопасных проверок прошли.
- Внутри этого запуска реальный PostgreSQL 16: 22 из 22 интеграционных тестов прошли без пропусков.
- Постоянный audit-ledger синхронизирован коммитом `1f4f52f` и содержит 82 теста.
- Windows native release gate `32624837780`: `PASS`; сборка, installer UI, hidden visual acceptance, interrupted-update recovery, изолированная установка, ярлык/иконка, SBOM и evidence прошли.
