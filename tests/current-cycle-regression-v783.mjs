...
const requiredGateCommands=[
  ...
  'node tests/license-invitation-accept-unit.mjs',
  'node tests/license-exact-permissions-runtime-unit.mjs',
  ...
  'node tests/entity-remote-apply-atomicity-unit.cjs',
...
assert(desktopPlatform.includes("if(environment===WAREHOUSE_REGISTRY_ENVIRONMENT)add('warehouse',warehouseId,serverWarehouseEntityPayloadV784(warehouse,{warehouseId,environment}))"));
assert(desktopPlatform.includes("warehouses?.({environment:'live'})"));
assert(desktopPlatform.includes("environment:WAREHOUSE_REGISTRY_ENVIRONMENT,commandId:newEntityCommandId(),authorUserId,changes"));
...
const localMigrationSchema=desktopPlatform.match(/const LOCAL_TO_SERVER_MIGRATION_SCHEMA_V783=(\d+);/);
assert(localMigrationSchema,'the durable local-to-server migration journal schema must be explicit');
assert.equal(Number(localMigrationSchema[1]),3,'schema v3 is required for outbox-bound, chunk-authenticated migration recovery');
assert(desktopPlatform.includes("const active=all.filter(item=>(item.state==='pending'||item.state==='sending')&&item.preserveLocal!==false)"),'v3 must include every durable pending/sending local command in the migrated snapshot');
assert(desktopPlatform.includes('expected=Array.isArray(journalEntry?.outboxCommandIds)?journalEntry.outboxCommandIds.map(String):active.map(item=>String(item.commandId))'),'v3 resume must use the command IDs frozen in the journal instead of silently adopting a changed outbox');
assert(desktopPlatform.includes('outboxCommandIds:migrationQueues.get(String(item.id))?.commandIds||[]'),'v3 journal must persist the exact outbox command set for every warehouse');
assert(desktopPlatform.includes('sourceSignatures:signatures'),'v3 journal must bind every warehouse to its effective snapshot fingerprint');
assert(desktopPlatform.includes("intent:{kind:'local_migration_import',targetId:id,snapshotFingerprint,chunkIndex:index,chunkCount:chunks.length}"),'every v3 entity chunk must carry its migration scope, snapshot fingerprint and complete chunk position');
assert(desktopPlatform.includes("existing.schemaVersion!==LOCAL_TO_SERVER_MIGRATION_SCHEMA_V783")&&desktopPlatform.includes("code:'LOCAL_MIGRATION_JOURNAL_INCOMPATIBLE'"),'an older or foreign migration journal must fail closed rather than be resumed as v3');
...
const migrationUploaded=desktopPlatform.indexOf("entry.state='uploaded'");
const migrationOutboxConfirmed=desktopPlatform.indexOf('includedInSnapshot:true');
const migrationActiveRestored=desktopPlatform.indexOf('const original=sourceIds.has(String(journal.originalActiveWarehouseId))');
const migrationCompleted=desktopPlatform.indexOf("journal={...journal,state:'complete'");
assert(migrationUploaded>=0&&migrationUploaded<migrationOutboxConfirmed&&migrationOutboxConfirmed<migrationActiveRestored&&migrationActiveRestored<migrationCompleted,'v3 may confirm local outbox commands only after upload, then restore the original warehouse before completing the journal');
...
assert(desktopPlatform.includes('setTimeout(async()=>{try{if(onlineEntitySyncAvailable())await bootstrapEntitySync();else{await restoreLocalOutboxOverlay();'),'online startup must use the authoritative bootstrap overlay and reserve standalone outbox restore for offline startup');
