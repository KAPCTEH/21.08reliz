...
assert.match(route,/p\.signature===routeSignature\(def\.orders,def\.id\)&&\(p\.engineSignature===routeEngineSignature\(def\)\|\|loadingPlanCompatible\(def,p\)\)/);
const engineSignatureContext=vm.createContext({ENGINE_VERSION:'test',asArray:value=>Array.isArray(value)?value:[],routeModeFor:()=> 'round',orderWarehouse:order=>order.warehouseId,warehouseId:()=> 'warehouse-1',hashString:value=>value,settings:{warehouse:{lat:59.9,lon:30.3},maxStops:12,maxRoundKm:1000,minRouteHours:8,maxRouteHours:11,serviceMinMinutes:10,serviceMaxMinutes:20,loadingStartTime:'08:00',loadingBayCount:2,loadingMinutes:40,loadingIntervalMinutes:10,driverArrivalLeadMinutes:15,arrivalWindowMinutes:10,loadingPriority:'distance_desc'}});
vm.runInContext(`${functionSource(route,'routeEngineSignature')};this.signature=routeEngineSignature`,engineSignatureContext);
const signatureOrder={id:'order-1',updatedAt:'2026-08-30T10:00:00.000Z',deliveryDate:'2026-08-30',deliveryAddress:'Невский проспект, 28',geo:{lat:59.9,lon:30.3,region:'Санкт-Петербург',district:'Центральный'},warehouseId:'warehouse-1',items:[{productId:'product-1',qty:2,volumeM3:.01,weightKg:4,lengthMm:300,widthMm:200,heightMm:100}]};
const stableSignature=engineSignatureContext.signature({id:'route-1',orders:[signatureOrder]});signatureOrder.updatedAt='2026-08-30T11:00:00.000Z';signatureOrder.fulfillmentStatus='picking';assert.equal(engineSignatureContext.signature({id:'route-1',orders:[signatureOrder]}),stableSignature);signatureOrder.deliveryAddress='Невский проспект, 30';assert.notEqual(engineSignatureContext.signature({id:'route-1',orders:[signatureOrder]}),stableSignature);
const loadingCompatibilityContext=vm.createContext({asArray:value=>Array.isArray(value)?value:[],routeAssignments:{'order-1':'route-1'},routeLocks:{'order-1':'route-1'}});vm.runInContext(`${functionSource(route,'loadingPlanCompatible')};this.compatible=loadingPlanCompatible`,loadingCompatibilityContext);const loadingDef={id:'route-1',orders:[{id:'order-1',fulfillmentStatus:'picking',warehouseFlowStatus:'picking'}]},loadingPlan={lifecycleStatus:'loading'};assert.equal(loadingCompatibilityContext.compatible(loadingDef,loadingPlan),true);loadingDef.orders[0].warehouseFlowStatus='reserved';assert.equal(loadingCompatibilityContext.compatible(loadingDef,loadingPlan),false);
const routeLockContext=vm.createContext({routeExecutions:{},routePlans:{},orders:[],routeAssignments:{},asArray:value=>Array.isArray(value)?value:[]});
vm.runInContext(`const activeExecution=id=>{const e=routeExecutions?.[id];return e&&['in_transit','awaiting_close'].includes(e.status)?e:null};${functionSource(route,'routeBuildLocked')};this.locked=routeBuildLocked`,routeLockContext);
assert.equal(routeLockContext.locked('route-1'),false);
routeLockContext.routePlans['route-1']={lifecycleStatus:'loading'};assert.equal(routeLockContext.locked('route-1'),true);
routeLockContext.routePlans['route-1']={lifecycleStatus:'ready_to_release'};routeLockContext.orders=[{id:'order-1',fulfillmentStatus:'picking',warehouseFlowStatus:'reserved'}];routeLockContext.routeAssignments['order-1']='route-1';assert.equal(routeLockContext.locked('route-1'),true);
routeLockContext.orders[0].fulfillmentStatus='active';routeLockContext.orders[0].warehouseFlowStatus='picking';assert.equal(routeLockContext.locked('route-1'),true);
assert.match(route,/protectedIds=new Set\(state\.allDefs\.filter\(def=>routeBuildLocked\(def\.id\)\)/);
assert.match(route,/if\(routeBuildLocked\(id\)\)\{alert\('Рейс уже находится в комплектации или выполнении/);
