...
window.JustFunDesktop = {
  ...
  audit: { event: async payload => { const detailCode=String(payload?.detail?.code||payload?.detail?.reason||''); window.__bridgeCalls.push(`audit:${String(payload?.action||'')}:${detailCode}:${String(payload?.correlationId||'')}`); return {ok:true,correlationId:String(payload?.correlationId||'')}; } },
  getActiveWarehousePreference: async () => ({
    ok: true,
    companyId: runtimeEdition === 'full' ? 'cmp_company_test_12345' : '',
    userId: runtimeEdition === 'full' ? `usr_${testServerRole}_test` : '',
    environment: 'live',
    warehouseId: ''
  }),
...
if(roleMatrixMode){
  const scope='teplitsa_company_cmp_company_test_12345__',warehouseId='warehouse-role-matrix',now='2026-08-30T00:00:00.000Z';
  const warehouse={id:warehouseId,name:'Склад проверки ролей',code:'ROLE',address:'Санкт-Петербург',lat:59.9343,lon:30.3351,timezone:'Europe/Moscow',status:'active',catalogMode:'empty',origin:'server',revision:1,digest:'A'.repeat(64),createdAt:now,updatedAt:now};
  window.localStorage.setItem(`${scope}warehouses_registry_v600`,JSON.stringify({version:2,activeWarehouseId:warehouseId,warehouses:[warehouse],createdAt:now,updatedAt:now,serverWorkspaceId:'cmp_company_test_12345',serverRegistryInitialized:true,serverAuthoritativeEmpty:false}));
}
...
if(localToServerMigration){
  ...
  window.localStorage.setItem(outboxKey,JSON.stringify({schemaVersion:1,dataContractVersion:3,scope:outboxScope,createdAt:now,updatedAt:now,entries:[{commandId:pendingCommandId,scope:outboxScope,companyId:'cmp_company_test_12345',warehouseId:first,environment:'live',intent:{kind:'order_not_relevant',targetId:storedOrder.id},changes:[{type:'orders',id:storedOrder.id,baseVersion:0,deleted:false,payload:pendingOrder}],state:'pending',createdAt:now,updatedAt:now,authorUserId:'usr_owner_test',deviceId:'desktop:migration-test',dataContractVersion:3,attempts:0,nextAttemptAt:null,lastError:null,confirmedAt:null,preserveLocal:true}]}));
  const dirtyKey=`jf.reg-entity-dirty.v1.${safeScope}`,dirtyOwnerKey=`jf.reg-entity-dirty-owner.v1.${safeScope}`;
  window.localStorage.setItem(dirtyKey,'1');
  window.localStorage.setItem(dirtyOwnerKey,JSON.stringify({schemaVersion:1,scope:outboxScope,ownerUserId:'usr_owner_test',deviceId:'desktop:migration-test',createdAt:now,updatedAt:now}));
  window.__migrationPendingOutbox={scope:outboxScope,commandId:pendingCommandId,dirtyKey,dirtyOwnerKey};
...
if(localToServerMigration){
  try{
    ...
    const initialExpected=localToServerMigrationResume?'failed':'complete';for(let attempt=0;attempt<200;attempt++){try{journal=JSON.parse(B.raw.get(migrationKey)||'null')}catch{}if(journal?.state===initialExpected&&(initialExpected!=='complete'||journal?.cleanupState==='complete'))break;await new Promise(resolve=>setTimeout(resolve,50))}
    let resumeReplay=null;if(localToServerMigrationResume){const lostCommand='client:migrate-v783:entities:warehouse-local-a:0',processedBefore=window.__processedEntityCommands.size,attemptsBefore=window.__entityCommandAttempts.filter(value=>value.endsWith(':'+lostCommand)).length;await window.JustFunWarehouseRegistryV783.refresh();for(let attempt=0;attempt<200;attempt++){try{journal=JSON.parse(B.raw.get(migrationKey)||'null')}catch{}if(journal?.state==='complete'&&journal?.cleanupState==='complete')break;await new Promise(resolve=>setTimeout(resolve,50))}const attemptsAfter=window.__entityCommandAttempts.filter(value=>value.endsWith(':'+lostCommand)).length;resumeReplay={state:journal?.state||null,processedBefore,processedAfter:window.__processedEntityCommands.size,attemptsBefore,attemptsAfter,replayed:window.__replayedEntityCommands.some(value=>value.endsWith(':'+lostCommand))}}
    for(let attempt=0;attempt<200;attempt++){const queue=window.JustFunLocalOutboxV783.create(window.localStorage,window.__migrationPendingOutbox.scope);if(queue.status().active===0&&window.localStorage.getItem(window.__migrationPendingOutbox.dirtyKey)===null&&window.localStorage.getItem(window.__migrationPendingOutbox.dirtyOwnerKey)===null)break;await new Promise(resolve=>setTimeout(resolve,50))}
    ...
    const dirtyJournalValue=window.localStorage.getItem(window.__migrationPendingOutbox.dirtyKey),dirtyOwnerValue=window.localStorage.getItem(window.__migrationPendingOutbox.dirtyOwnerKey);
    localToServerMigrationResult={journalState:journal?.state||null,cleanupState:journal?.cleanupState||null,warehouses:serverWarehouses.length,orderMigrated:Boolean(order),pendingOverlayMigrated:order?.payload?.fulfillmentStatus==='not_relevant'&&order?.payload?.archived===true,pendingOutboxConfirmed:pendingEntry?.state==='confirmed'&&pendingEntry?.preserveLocal===false,pendingOutboxState:pendingEntry?.state||null,pendingOutboxPreserveLocal:pendingEntry?.preserveLocal??null,pendingOutboxActive:pendingQueue.status().active,pendingCommandJournaled:journalWarehouse?.outboxCommandIds?.includes(window.__migrationPendingOutbox.commandId)===true,dirtyGenerationAtStart:journalWarehouse?.dirtyGenerationAtStart??null,dirtyJournalCleared:dirtyJournalValue===null&&dirtyOwnerValue===null,dirtyJournalValue,dirtyOwnerPresent:dirtyOwnerValue!==null,productMigrated:Boolean(product),registryServerOwned:registry.warehouses.length===2&&registry.warehouses.every(item=>item.origin==='server'),activeWarehousePreserved:registry.activeWarehouseId==='warehouse-local-a',secondCatalogMode:registry.warehouses.find(item=>item.id==='warehouse-local-b')?.catalogMode||null,resumeReplay};
    ...
    if(localToServerMigrationResult.journalState!=='complete'||localToServerMigrationResult.cleanupState!=='complete'||localToServerMigrationResult.warehouses!==2||!localToServerMigrationResult.orderMigrated||!localToServerMigrationResult.pendingOverlayMigrated||!localToServerMigrationResult.pendingOutboxConfirmed||!localToServerMigrationResult.pendingCommandJournaled||!localToServerMigrationResult.dirtyJournalCleared||!localToServerMigrationResult.productMigrated||!localToServerMigrationResult.registryServerOwned||!localToServerMigrationResult.activeWarehousePreserved||localToServerMigrationResult.secondCatalogMode!=='empty'||!resumeOk)errors.push({phase:'local-to-server-migration',level:'error',text:JSON.stringify(localToServerMigrationResult)});
