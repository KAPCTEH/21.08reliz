...
const testEdition = process.env.JF_TEST_EDITION === 'full' ? 'full' : 'demo';
...
if(localToServerMigration){
  try{
    ...
    localToServerMigrationResult={journalState:journal?.state||null,warehouses:serverWarehouses.length,orderMigrated:Boolean(order),pendingOverlayMigrated:order?.payload?.fulfillmentStatus==='not_relevant'&&order?.payload?.archived===true,pendingOutboxConfirmed:pendingEntry?.state==='confirmed'&&pendingEntry?.preserveLocal===false,pendingCommandJournaled:journalWarehouse?.outboxCommandIds?.includes(window.__migrationPendingOutbox.commandId)===true,productMigrated:Boolean(product),registryServerOwned:registry.warehouses.length===2&&registry.warehouses.every(item=>item.origin==='server'),activeWarehousePreserved:registry.activeWarehouseId==='warehouse-local-a',secondCatalogMode:registry.warehouses.find(item=>item.id==='warehouse-local-b')?.catalogMode||null,resumeReplay:null};
