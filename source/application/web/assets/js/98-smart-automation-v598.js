...
(function(){
  ...
  function renderSmartProgramHealth(){const box=$('smartProgramHealth');if(!box)return;const last=settings.program?.lastBackupAt,days=last?Math.floor((Date.now()-new Date(last).getTime())/86400000):null,storageCount=orders.length+products.length+drivers.length+inventoryMovements.length;box.innerHTML=`<div><span>Часовой пояс</span><b>Москва, UTC+3</b></div><div><span>Последняя копия</span><b>${last?`${formatDateTime(last)} · ${days} дн. назад`:'ещё не создавалась'}</b></div><div><span>Объектов в локальной базе</span><b>${storageCount.toLocaleString('ru-RU')}</b></div><div><span>Автосохранение</span><b>включено для всех рабочих разделов</b></div>`}
  window.saveSmartProgramSettings=function(){settings.program={...smartDefaultProgram(),automationLevel:$('programAutomationLevel')?.value||'balanced',interfaceDensity:$('programInterfaceDensity')?.value||'comfortable',backupReminderDays:clamp(Number($('programBackupDays')?.value||7),1,90),defaultView:$('programDefaultView')?.value||'orders',autoOpenIssues:!!$('programAutoOpenIssues')?.checked,decisionExplanations:!!$('programDecisionExplanations')?.checked,autosaveIndicator:!!$('programAutosaveIndicator')?.checked,compactLargeLists:!!$('programCompactLargeLists')?.checked,lastBackupAt:settings.program?.lastBackupAt||'',demoCargoTemplatesVersion:Number(settings.program?.demoCargoTemplatesVersion||0),timezone:MSK_TZ};persistSettings();applyProgramAppearance();renderSmartProgramHealth();alert('Настройки поведения программы сохранены.')}
