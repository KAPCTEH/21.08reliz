...
function renderNoWarehouse(message='Для вашей учётной записи не назначено ни одного доступного склада. Обратитесь к владельцу или администратору.'){
  const canCreate=canCreateWarehouseFromNoAccessV783(),first=registry().serverAuthoritativeEmpty===true,createLabel=first?'Создать первый склад':'Создать новый склад',quarantined=hasEntityPermissionQuarantine();
  authFrame(`<div class="jf-no-access"><h2>${canCreate?(first?'Складов пока нет':'Нет активного склада'):'Склад не назначен'}</h2><p id="jfNoWarehouseMessage">${esc(canCreate?(first?'Сервер подтвердил пустой реестр компании. Создайте первый склад — локальный склад по умолчанию восстановлен не будет.':'Все склады сейчас находятся в архиве. Создайте новый склад или обратитесь к владельцу.'):message)}</p></div><div class="jf-auth-actions">${canCreate?`<button class="jf-auth-button" id="jfCreateFirstWarehouse">${createLabel}</button>`:''}${quarantined?'<button class="jf-auth-button secondary" id="jfExportQuarantine">Экспортировать локальную копию</button>':''}<button class="jf-auth-button secondary" id="jfRetry">Повторить проверку</button><button class="jf-auth-button${canCreate?' secondary':''}" id="jfLogout">Выйти</button></div>`,'Доступ ограничен');
...
function blockWorkspaceForEntityPermissionQuarantine(message){clearTimeout(cloudSyncState?.uploadTimer);clearInterval(cloudSyncState?.pollTimer);if(cloudSyncState){cloudSyncState.uploadTimer=null;cloudSyncState.pollTimer=null;cloudSyncState.dirty=true;try{persistEntityDirty(true)}catch{}}document.documentElement.classList.remove('jf-authenticated');renderNoWarehouse(message)}
...
async function logout(){try{assertEntityContextChangeAllowed({kind:'logout'})}catch(error){toast(error?.message||String(error),'error');audit('logout_blocked_critical_operation',{code:error?.code||''});return false}audit('logout');stopTelegramPolling();clearSession();if(desktopSession?.edition==='demo'){await window.JustFunDesktop?.quit?.();return true}await window.JustFunDesktop?.auth?.logout?.();currentUser=null;users=[];desktopSession.auth=null;await window.JustFunDesktop?.restart?.();return true}
...
function localMigrationEffectiveSnapshotV783(companyId,warehouseId,storedSnapshot,journalEntry=null){
  ...
  return{snapshot,queue,commandIds:expected}
}
...
async function migrateLocalCompanyToEmptyServerV783(B,local,response,remote){
  ...
  if(!journal){journal={schemaVersion:LOCAL_TO_SERVER_MIGRATION_SCHEMA_V783,workspaceId:companyId,state:'prepared',createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),originalActiveWarehouseId:String(local.activeWarehouseId||''),sourceSignatures:signatures,warehouses:sourceWarehouses.map(item=>({id:String(item.id),code:String(item.code||''),state:'prepared',createCommandId:localMigrationCommandIdV783('warehouse',item.id),chunkCommandIds:[],outboxCommandIds:migrationQueues.get(String(item.id))?.commandIds||[]}))};writeLocalToServerMigrationV783(B,journal)}
...
async function synchronizeCompanyWarehouseRegistry(){
  ...
  await recoverPendingWarehouseWritesV784();
...
async function enterWorkspace(){
  try{await recoverCriticalEntityMutation()}catch(error){audit('critical_recovery_startup_blocked',{code:error?.code||'',error:String(error?.message||error)});renderNoWarehouse('Аварийное восстановление данных не завершено. Работа заблокирована до диагностики хранилища.');return false}const allowed=allowedWarehouseIds(),current=activeWarehouseId();
...
function persistEntityDirty(value,scope=cloudSyncState.scope||entityScope()){try{if(value){const current=entityDirtyGeneration(scope),next=current>=Number.MAX_SAFE_INTEGER?1:current+1;localStorage.setItem(entityDirtyStorageKey(scope),String(next))}else localStorage.removeItem(entityDirtyStorageKey(scope));return true}catch(error){if(value)throw outboxError('ENTITY_DIRTY_JOURNAL_WRITE_FAILED','Не удалось зафиксировать неотправленное изменение на диске.');return false}}
...
function resetEntityScope(){
  ...
  try{if(!cloudSyncState.outbox){if(!window.JustFunLocalOutboxV783?.create)throw outboxError('OUTBOX_MODULE_MISSING','Модуль локальной очереди не загружен.');cloudSyncState.outbox=window.JustFunLocalOutboxV783.create(localStorage,scope);if(cloudSyncState.outbox.isCorrupt())throw cloudSyncState.outbox.corruption();cloudSyncState.outboxes.set(scope,cloudSyncState.outbox)}}catch(error){cloudSyncState.outbox=null;cloudSyncState.outboxError=error instanceof Error?error:outboxError('OUTBOX_INIT_FAILED',String(error))}
  cloudSyncState.dirty=Boolean(cloudSyncState.outbox&&!cloudSyncState.outboxError&&cloudSyncState.outbox.status().active)||durableEntityDirty(scope);rememberLocalEntityBaseline();rememberObservedEntitySnapshot();renderLocalOutboxStatus();
}
function saveEntitySyncState({required=false}={}){
  resetEntityScope();
  try{localStorage.setItem(entityStateStorageKey(),JSON.stringify({cursor:cloudSyncState.cursor,entities:Object.fromEntries(cloudSyncState.known),conflicts:Object.fromEntries(cloudSyncState.conflicts),readableTypes:[...cloudSyncState.readableTypes],savedAt:new Date().toISOString()}));return true}catch(error){console.error('VPS entity state',error);if(required)throw outboxError('ENTITY_STATE_WRITE_FAILED','Подтверждение VPS получено, но его нельзя надёжно зафиксировать на этом компьютере. Команда будет повторена с тем же идентификатором.');return false}
}
...
function splitEntitySnapshot(snapshot,{warehouseId=activeWarehouseId(),environment=activeEnvironment()}={}){
  ...
  if(environment===WAREHOUSE_REGISTRY_ENVIRONMENT)add('warehouse',warehouseId,warehouse);
...
function requireWritableLocalEntityChanges(changes,context={},reason='local_dirty_recovery'){
  const denied=asArray(changes).filter(change=>!canWriteEntity(change.type)),key=entityPermissionQuarantineKey(context);if(!denied.length){try{localStorage.removeItem(key)}catch{}return changes}
  const scope=`${String(context.companyId||desktopSession?.auth?.company?.id||'unknown')}:${String(context.environment||activeEnvironment())}:${String(context.warehouseId||activeWarehouseId())}`;persistEntityDirty(true,scope);cloudSyncState.dirty=true;const details={reason,companyId:String(context.companyId||desktopSession?.auth?.company?.id||''),warehouseId:String(context.warehouseId||activeWarehouseId()),environment:String(context.environment||activeEnvironment()),entities:denied.slice(0,1000).map(change=>({type:String(change.type),id:String(change.id),deleted:change.deleted===true})),total:denied.length,exportRequired:true};try{localStorage.setItem(key,JSON.stringify({...details,recordedAt:new Date().toISOString()}))}catch{}audit('local_changes_quarantined_after_permission_change',{reason,types:[...new Set(denied.map(change=>String(change.type)))],total:denied.length,warehouseId:details.warehouseId,environment:details.environment});const error=outboxError('ENTITY_LOCAL_CHANGES_PERMISSION_REVOKED','На компьютере найдены несинхронизированные изменения, на которые у текущей роли больше нет права записи. Серверный снимок не применён: локальные данные сохранены. Войдите с подходящей ролью и повторите синхронизацию либо экспортируйте резервную копию.',details);blockWorkspaceForEntityPermissionQuarantine(error.message);throw error
}
...
const WAREHOUSE_LIFECYCLE_SCHEMA_V784=1,WAREHOUSE_LIFECYCLE_STORE_V784='journal',WAREHOUSE_LIFECYCLE_RECORD_V784='active';
...
function warehouseLifecycleReadIndexV784(companyId){
  const key=warehouseLifecycleIndexKeyV784(companyId);let value;try{value=JSON.parse(localStorage.getItem(key)||'[]')}catch(error){throw warehouseLifecycleErrorV784('WAREHOUSE_LIFECYCLE_INDEX_CORRUPT','Журнал операций со складами повреждён. Автоматическая работа остановлена.',error)}if(!Array.isArray(value)||value.length>200)throw warehouseLifecycleErrorV784('WAREHOUSE_LIFECYCLE_INDEX_CORRUPT','Журнал операций со складами имеет неверный формат.');const seen=new Set();return value.map(item=>{const warehouseId=String(item?.warehouseId||''),commandId=String(item?.commandId||''),state=String(item?.state||'');if(!/^[A-Za-z0-9_-]{1,160}$/.test(warehouseId)||!/^[A-Za-z0-9_.:-]{16,180}$/.test(commandId)||!['preparing','ready','clearing'].includes(state)||seen.has(warehouseId))throw warehouseLifecycleErrorV784('WAREHOUSE_LIFECYCLE_INDEX_CORRUPT','Журнал операций со складами содержит недопустимую запись.');seen.add(warehouseId);return{warehouseId,commandId,state,fingerprint:String(item?.fingerprint||''),updatedAt:String(item?.updatedAt||'')}})}
...
function warehouseLifecycleFingerprintV784(warehouseId,warehouseCode,environment,changes){return entityFingerprint({warehouseId,warehouseCode,environment,changes})}
function canonicalWarehouseLifecycleV784(input,companyId,warehouseId){const source=asObject(input),environment=String(source.environment||''),commandId=String(source.commandId||''),warehouseCode=String(source.warehouseCode||''),changes=asArray(source.changes).map(item=>cloneValue(item)),fingerprint=String(source.fingerprint||'');if(Number(source.schemaVersion)!==WAREHOUSE_LIFECYCLE_SCHEMA_V784||String(source.companyId||'')!==companyId||String(source.warehouseId||'')!==warehouseId||environment!=='live'||!/^[A-Za-z0-9_.:-]{16,180}$/.test(commandId)||!changes.length||changes.length>3||fingerprint!==warehouseLifecycleFingerprintV784(warehouseId,warehouseCode,environment,changes))throw warehouseLifecycleErrorV784('WAREHOUSE_LIFECYCLE_JOURNAL_CORRUPT','Журнал операции со складом повреждён или относится к другой области данных.');return{...source,id:WAREHOUSE_LIFECYCLE_RECORD_V784,schemaVersion:WAREHOUSE_LIFECYCLE_SCHEMA_V784,companyId,warehouseId,environment,commandId,warehouseCode,changes,fingerprint}}
async function prepareWarehouseLifecycleV784(journal){const companyId=String(journal.companyId),warehouseId=String(journal.warehouseId),pointer={warehouseId,commandId:String(journal.commandId),fingerprint:String(journal.fingerprint),state:'preparing'};warehouseLifecycleSetPointerV784(companyId,pointer);await warehouseLifecycleDbOperationV784(companyId,warehouseId,'readwrite',store=>store.put(journal));warehouseLifecycleSetPointerV784(companyId,{...pointer,state:'ready'});return journal}
async function clearWarehouseLifecycleV784(companyId,warehouseId,commandId,fingerprint){warehouseLifecycleSetPointerV784(companyId,{warehouseId,commandId,fingerprint,state:'clearing'});await warehouseLifecycleDbOperationV784(companyId,warehouseId,'readwrite',store=>store.delete(WAREHOUSE_LIFECYCLE_RECORD_V784));warehouseLifecycleRemovePointerV784(companyId,warehouseId);return true}
async function sendWarehouseLifecycleJournalV784(journal){
  let lastError;for(let attempt=0;attempt<2;attempt++){try{const result=await window.JustFunDesktop.regVps.writeWarehouse({warehouseId:journal.warehouseId,warehouseCode:journal.warehouseCode,environment:journal.environment,commandId:journal.commandId,changes:journal.changes});if(!result?.ok){const rejection=Object.assign(new Error(result?.error||'VPS не подтвердил изменение склада.'),{code:result?.code||'WAREHOUSE_WRITE_FAILED',details:result?.details||{}});if(definitiveEntityRejection(result)&&!retryableEntityFailure(rejection)){await clearWarehouseLifecycleV784(journal.companyId,journal.warehouseId,journal.commandId,journal.fingerprint);rejection.warehouseWriteRejected=true}throw rejection}validateEntityBatchAck(result,journal.changes,journal.commandId);await clearWarehouseLifecycleV784(journal.companyId,journal.warehouseId,journal.commandId,journal.fingerprint);return result}catch(error){if(error?.warehouseWriteRejected===true)throw error;lastError=error;if(attempt===0)continue}}
...
async function writeAuthoritativeWarehouse(record,{deleted=false,baseVersion,initialSettings=null,initialCompany=null}={}){
  ...
  const companyId=warehouseLifecycleCompanyV784(),warehouseCode=String(record?.code||''),fingerprint=warehouseLifecycleFingerprintV784(id,warehouseCode,WAREHOUSE_REGISTRY_ENVIRONMENT,changes),result=await serializeWarehouseLifecycleV784(async()=>{const recovered=await recoverPendingWarehouseWritesUnlockedV784(),matching=recovered.find(item=>item.warehouseId===id&&item.fingerprint===fingerprint);if(matching)return matching.result;const journal=canonicalWarehouseLifecycleV784({id:WAREHOUSE_LIFECYCLE_RECORD_V784,schemaVersion:WAREHOUSE_LIFECYCLE_SCHEMA_V784,companyId,warehouseId:id,warehouseCode,environment:WAREHOUSE_REGISTRY_ENVIRONMENT,commandId:newEntityCommandId(),changes,fingerprint,preparedAt:new Date().toISOString()},companyId,id);await prepareWarehouseLifecycleV784(journal);return sendWarehouseLifecycleJournalV784(journal)});
...
function overlayLocalOutbox(snapshot,queue=requireLocalOutbox()){
  let applied=0;
...
function capturePreBootstrapLocalIntent(baseline,current,{queue,knownEntities,context}){
  const expected=cloneValue(baseline);overlayLocalOutbox(expected,queue);const detected=entityChangesBetween(expected,current,{includeOutbox:true,knownEntities,queue});
  const blocked=queue.blockedEntityKeys(),blockedChange=detected.find(change=>blocked.has(entityKey(change.type,change.id)));if(blockedChange)throw outboxError('OUTBOX_ENTITY_BLOCKED',`Запись ${blockedChange.type}/${blockedChange.id} уже требует разрешения конфликта. Серверный снимок не применён, локальные данные сохранены.`);
...
async function restoreLocalOutboxOverlay(){
  ...
  cloudSyncState.suspended++;try{await window.TeplitsaWarehouseV600.importServerSnapshot(localSnapshot);await window.TeplitsaWarehouseV600?.whenPersisted?.();rememberLocalEntityBaseline(localSnapshot)}finally{cloudSyncState.suspended--}
  cloudSyncState.dirty=true;persistEntityDirty(true);window.renderAll?.();renderLocalOutboxStatus();audit('local_outbox_overlay_restored',{commands:status.active,changes:applied,warehouseId:activeWarehouseId(),environment:activeEnvironment()});return true
}
async function bootstrapEntitySync(force=false){
  ...
  operation=(async()=>{
    ...
    try{
      ...
      for(const entity of entities)serverKnown.set(entityKey(entity.type,entity.id),{version:Number(entity.version)||0,digest:String(entity.digest_sha256||''),fingerprint:entityFingerprint(entity.payload),deleted:false,eventId:Number(entity.event_id)||0});
      await window.TeplitsaWarehouseV600?.whenPersisted?.();assertEntityScope(expectedScope,expectedEpoch);capturePreBootstrapLocalIntent(baselineSnapshot,cloneValue(buildBackupPayload()),{queue,knownEntities:knownAtStart,context});assertEntityScope(expectedScope,expectedEpoch);
      cloudSyncState.known=serverKnown;cloudSyncState.conflicts=new Map();cloudSyncState.cursor=Number(result.cursor)||0;cloudSyncState.readableTypes=new Set(readableTypes);const overlaid=overlayLocalOutbox(serverSnapshot,queue);
      if(typeof buildBackupPayload==='function'&&typeof window.TeplitsaWarehouseV600?.importServerSnapshot==='function'){
        assertEntityScope(expectedScope,expectedEpoch);cloudSyncState.suspended++;try{await window.TeplitsaWarehouseV600.importServerSnapshot(serverSnapshot);assertEntityScope(expectedScope,expectedEpoch);rememberLocalEntityBaseline(serverSnapshot);rememberObservedEntitySnapshot()}finally{cloudSyncState.suspended--}
      }
      cloudSyncState.bootstrapped=true;cloudSyncState.pollFailures=0;cloudSyncState.nextPollAt=0;settleEntityDirty(expectedScope,queue,{generationAtStart:dirtyGenerationAtStart,serialAtStart:bootstrapSerial});saveEntitySyncState();renderLocalOutboxStatus();
    ...
    }catch(error){if(error?.code==='ENTITY_SCOPE_CHANGED')return false;throw error}
...
function buildPendingEntityChanges({snapshot=null,knownEntities=cloudSyncState.known,conflicts=cloudSyncState.conflicts,queue=requireLocalOutbox(),context=null}={}){
  const splitOptions=context?{warehouseId:String(context.warehouseId||''),environment:String(context.environment||'live')}:undefined,current=splitEntitySnapshot(snapshot||buildBackupPayload(),splitOptions),queued=latestQueuedEntityChanges(queue),changes=[],keys=new Set([...current.keys(),...knownEntities.keys(),...queued.keys()]);
  for(const key of keys){if(conflicts.has(key))continue;const entity=current.get(key),known=knownEntities.get(key),pending=queued.get(key),split=key.indexOf(':'),type=entity?.type||key.slice(0,split),entityId=entity?.id||key.slice(split+1);let change=null;
    ...
    else{if(pending?.deleted===true||(!pending&&(!known||known.deleted)))continue;change={type,id:entityId,baseVersion:nextLocalBaseVersion(type,entityId,known?.version,queue),deleted:true,payload:null,_fingerprint:''}}
    changes.push(change)
  }
  return requireWritableLocalEntityChanges(changes,context||{companyId:String(desktopSession?.auth?.company?.id||''),warehouseId:activeWarehouseId(),environment:activeEnvironment()},'durable_dirty_recovery');
}
...
function acceptEntityBatchResult(result,changes,expectedCommandId,expectedScope=cloudSyncState.scope,expectedEpoch=cloudSyncState.scopeEpoch){
  ...
  try{
    for(const item of asArray(result.entities)){
      ...
      cloudSyncState.known.set(entityKey(item.type,item.id),{version:Number(item.version)||0,digest:String(item.digest||''),fingerprint:item.deleted?'':pending?entityFingerprint(pending.payload):'',deleted:item.deleted===true,eventId:Number(item.eventId)||0})
    }
...
async function drainLocalOutboxNow({targetCommandId='',force=false,expectedScope='',expectedEpoch=null}={}){
  ...
  beginEntityInFlight(expectedScope,expectedEpoch);let targetState='pending',targetEntry=targetCommandId?queue.get(targetCommandId):null;
  try{
...
function reportCloudSyncFailure(error){
  ...
  try{persistEntityDirty(true)}catch{}
...
async function pollCloudRevision(){
  ...
  try{
    ...
    if(applied&&workingSnapshot){assertEntityScope(expectedScope,expectedEpoch);cloudSyncState.suspended++;try{await window.TeplitsaWarehouseV600?.importServerSnapshot?.(workingSnapshot);assertEntityScope(expectedScope,expectedEpoch);rememberLocalEntityBaseline(workingSnapshot)}finally{cloudSyncState.suspended--}integrationBadge('jfRegBadge','Получены изменения','ready');integrationStatus('jfRegStatus',`Применено ${applied} изменений отдельных записей с других компьютеров.`,'ok')}
