...
function capturePreBootstrapLocalIntent(baseline,current,{queue,knownEntities,context,serverEntities=[]}){
  const expected=cloneValue(baseline);overlayLocalOutbox(expected,queue,{conflicts:new Map(cloudSyncState.conflicts)});const detected=entityChangesBetween(expected,current,{includeOutbox:true,knownEntities,queue,context}).filter(change=>entityTypeWasReadableOrWritable(change.type)&&!serverEquivalentWarehouseMetadataChangeV784(change,serverEntities,context));
...
function buildPendingEntityChanges({snapshot=null,knownEntities=cloudSyncState.known,conflicts=cloudSyncState.conflicts,queue=requireLocalOutbox(),context=null,allowInferredDeletes=true,suppressedDeletes=null}={}){
  ...
  try{
    ...
    while(more&&rounds++<8){
      ...
      for(const rawEvent of events){const event=canonicalServerEntity(rawEvent),key=entityKey(event.type,event.id),known=stagedKnown.get(key),local=current.get(key),localDirty=local?(!known||known.deleted||local.fingerprint!==known.fingerprint):Boolean(known&&!known.deleted);if(Number(event.version)<=Number(known?.version||0))continue;metadataChanged=true;
        ...
        applyEntityToSnapshot(workingSnapshot,event,event.operation==='delete');if(event.operation==='delete')current.delete(key);else current.set(key,{type:event.type,id:event.id,payload:event.payload,fingerprint:entityFingerprint(event.payload)});stagedKnown.set(key,{version:Number(event.version)||0,digest:String(event.digest||''),fingerprint:event.operation==='delete'?'':entityFingerprint(event.payload),deleted:event.operation==='delete',eventId:Number(event.eventId)||0});applied++}
