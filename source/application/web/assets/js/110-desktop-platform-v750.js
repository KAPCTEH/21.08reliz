...
async function captureDirtyLocalChangesBeforeAccessRefresh(context,queue){
  ...
  enqueueBackgroundSnapshot(queue,cloneValue(buildBackupPayload()),{knownEntities:new Map(cloudSyncState.known),conflicts:new Map(cloudSyncState.conflicts),context,kind:'access_refresh_local_capture'});renderLocalOutboxStatus()
}
...
async function refreshLiveAccess(reason='periodic'){
  ...
  liveAccessRefreshPromise=(async()=>{
    ...
    await confirmActiveWarehouseContext();
    if(!document.documentElement.classList.contains('jf-authenticated')&&allowed.length)mountWorkspace();
    else{addDesktopStrip();applyPermissions();installUserManagement();if(q('#jfUsersBox'))renderUsersPanel().catch(()=>{});window.renderAll?.()}
...
function assertEntityRecoveryOwnership({scope=cloudSyncState.scope||entityScope(),queue=scope===cloudSyncState.scope?cloudSyncState.outbox:null,journalOwnerUserId=null,legacyJournal=false,block=true,adoptLegacyOwner=true}={}){
  scope=String(scope);const currentUserId=currentEntityUserId();if(!validEntityRecoveryUserId(currentUserId))return entityRecoveryOwnershipFailure('ENTITY_LOCAL_RECOVERY_USER_UNKNOWN','',currentUserId,'current-user',block);
  ...
  let owners;try{owners=entityOutboxOwnerIds(queue)}catch(error){return entityRecoveryOwnershipFailure(String(error?.code||'ENTITY_OUTBOX_OWNER_CORRUPT'),'',currentUserId,'outbox',block)}if(owners.size>1)return entityRecoveryOwnershipFailure('ENTITY_OUTBOX_MULTIPLE_OWNERS','',currentUserId,'outbox',block);const outboxOwner=owners.size?[...owners][0]:'';if(outboxOwner&&outboxOwner!==currentUserId)return entityRecoveryOwnershipFailure('ENTITY_OUTBOX_OWNER_MISMATCH',outboxOwner,currentUserId,'outbox',block);
  const generation=entityDirtyGeneration(scope),protectedLocal=Boolean(generation||outboxOwner);let ownerRecord=null;try{ownerRecord=readEntityDirtyOwner(scope)}catch(error){return entityRecoveryOwnershipFailure(String(error?.code||'ENTITY_DIRTY_OWNER_CORRUPT'),'',currentUserId,'dirty-owner',block)}let ownerUserId=protectedLocal?String(ownerRecord?.ownerUserId||outboxOwner||''):'';
...
function overlayLocalOutbox(snapshot,queue=requireLocalOutbox(),options={}){
  let applied=0;const markers=Array.isArray(options.resolutionMarkers)?options.resolutionMarkers:null,authoritativeKnown=options.authoritativeKnown instanceof Map?options.authoritativeKnown:null,authoritativeSnapshot=options.authoritativeSnapshot===true,activeConflictKeys=new Set();
  ...
  for(const entry of queue.overlayEntries()){const conflictKeys=new Set(entry.state==='conflict'?asArray(queue.conflictEntityKeys?.(entry.commandId)):entry.state==='rejected'?entry.changes.map(change=>entityKey(change.type,change.id)):[]);for(const change of entry.changes){applyEntityToSnapshot(snapshot,change,change.deleted===true);applied++;const key=entityKey(change.type,change.id);if(conflictKeys.has(key)){activeConflictKeys.add(key);cloudSyncState.conflicts.set(key,{type:change.type,id:change.id,commandId:entry.commandId,state:entry.state,...asObject(entry.lastError),detectedAt:entry.updatedAt})}}
  }
  for(const[key,value]of cloudSyncState.conflicts)if(value?.commandId&&!activeConflictKeys.has(key))cloudSyncState.conflicts.delete(key);return applied
}
function applyPendingResolutionMetadata(markers,authoritativeKnown=null){
  for(const entry of markers){const remote=asObject(entry.serverResult),key=entityKey(remote.type,remote.id),authoritative=authoritativeKnown instanceof Map?authoritativeKnown.get(key):null;if(authoritative&&Number(authoritative.version)>=Number(remote.version)){cloudSyncState.conflicts.delete(key);continue}const current=cloudSyncState.known.get(key);if(current&&Number(current.version)>Number(remote.version))throw outboxError('OUTBOX_RESOLUTION_METADATA_STALE','Локальное служебное состояние новее журнала выбора. Автоматическая перезапись остановлена.');cloudSyncState.known.set(key,{version:Number(remote.version)||0,digest:String(remote.digest||''),fingerprint:remote.deleted?'':entityFingerprint(remote.payload),deleted:remote.deleted===true,eventId:Number(remote.eventId)||0});cloudSyncState.conflicts.delete(key)}
}
...
function reconcileServerEquivalentWarehouseOutboxV784(queue,entities,context){
  ...
  const blockingWarehouseEntry=queue.list().some(entry=>entry?.preserveLocal===true&&(entry?.state==='conflict'||entry?.state==='rejected')&&asArray(entry?.changes).some(change=>change?.type==='warehouse'&&String(change?.id||'')===warehouseId));if(prefix.length&&!blockingWarehouseEntry)cloudSyncState.conflicts.delete(entityKey('warehouse',warehouseId));
...
function capturePreBootstrapLocalIntent(baseline,current,{queue,knownEntities,context}){
  const expected=cloneValue(baseline);overlayLocalOutbox(expected,queue);const detected=entityChangesBetween(expected,current,{includeOutbox:true,knownEntities,queue}).filter(change=>entityTypeWasReadableOrWritable(change.type));
...
async function bootstrapEntitySync(force=false){
  ...
  operation=(async()=>{
    ...
    try{
      ...
      if(cloudSyncState.dirty&&!pendingResolutionsAtStart.length){const recoveryKnown=recoveryKnownEntitiesFromServer(localSnapshot,serverSnapshot,knownAtStart,serverKnown,context),pendingBeforeServer=buildPendingEntityChanges({snapshot:localSnapshot,knownEntities:recoveryKnown,conflicts:new Map(cloudSyncState.conflicts),queue,context});for(let offset=0;offset<pendingBeforeServer.length;offset+=1000)queue.enqueue(localOutboxEntry({kind:'durable_dirty_recovery',targetId:warehouseId},pendingBeforeServer.slice(offset,offset+1000),context))}
      await window.TeplitsaWarehouseV600?.whenPersisted?.();assertEntityScope(expectedScope,expectedEpoch);if(!pendingResolutionsAtStart.length)capturePreBootstrapLocalIntent(baselineSnapshot,cloneValue(buildBackupPayload()),{queue,knownEntities:knownAtStart,context});assertEntityScope(expectedScope,expectedEpoch);reconcileServerEquivalentWarehouseOutboxV784(queue,entities,context);
      cloudSyncState.known=serverKnown;cloudSyncState.conflicts=new Map();cloudSyncState.cursor=Number(result.cursor)||0;cloudSyncState.readableTypes=new Set(readableTypes);const resolutionMarkers=[];resolutionApplyStarted=pendingResolutionsAtStart.length>0;const overlaid=overlayLocalOutbox(serverSnapshot,queue,{resolutionMarkers,authoritativeKnown:serverKnown,authoritativeSnapshot:true});
      if(typeof buildBackupPayload==='function'&&typeof window.TeplitsaWarehouseV600?.importServerSnapshot==='function'){
        assertEntityScope(expectedScope,expectedEpoch);cloudSyncState.suspended++;try{const imported=await window.TeplitsaWarehouseV600.importServerSnapshot(serverSnapshot),persisted=await window.TeplitsaWarehouseV600?.whenPersisted?.();assertEntityScope(expectedScope,expectedEpoch);if(imported===false||persisted===false||window.__warehousePersistenceCritical)throw outboxError('ENTITY_BOOTSTRAP_IMPORT_NOT_DURABLE','Серверный снимок не подтверждён локальным диском.');rememberLocalEntityBaseline(serverSnapshot);rememberObservedEntitySnapshot()}finally{cloudSyncState.suspended--}
      }
      applyPendingResolutionMetadata(resolutionMarkers,serverKnown);cloudSyncState.bootstrapped=true;cloudSyncState.pollFailures=0;cloudSyncState.nextPollAt=0;saveEntitySyncState({required:resolutionMarkers.length>0});finalizePendingServerResolutions(queue,resolutionMarkers);settleEntityDirty(expectedScope,queue,{generationAtStart:dirtyGenerationAtStart,serialAtStart:bootstrapSerial});renderLocalOutboxStatus();
