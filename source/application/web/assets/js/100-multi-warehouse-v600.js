...
function applyBranding(){
  ...
  if(logo){const visible=c.showLogoInApp!==false,logoDataUrl=safeLogoDataUrl(c.logoDataUrl),custom=visible&&!!logoDataUrl;logo.classList.toggle('warehouse-custom-logo',custom);logo.setAttribute('aria-hidden',String(!visible));if(visible)logo.style.removeProperty('display');else logo.style.setProperty('display','none','important');if(custom)logo.style.setProperty('background-image',`url("${logoDataUrl}")`,'important');else logo.style.removeProperty('background-image')}
