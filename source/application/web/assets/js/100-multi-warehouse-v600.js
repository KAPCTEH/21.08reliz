...
function applyBranding(){
  ...
  const title=document.querySelector('.brand-title'),sub=document.querySelector('.brand-sub'),logo=document.querySelector('.company-brand .logo');
...
exportBackup__implV595=function(){const payload=buildBackupPayload(),code=activeCode(),env=isDemonstrationMode()?'DEMO':'WORK',blob=new Blob([JSON.stringify(payload,null,2)],{type:'application/json;charset=utf-8'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`${code}_${env}_backup_${todayISO()}.json`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},1000);settings.program=settings.program||{};settings.program.lastBackupAt=nowIso();persistSettings()};
...
importBackupFile__implV595=async function(event){
  ...
  try{
    ...
    exportBackup();applyWarehouseSnapshotV783(parsed);alert('Резервная копия восстановлена только в активный склад и текущую среду. Остальные данные не изменялись.')
